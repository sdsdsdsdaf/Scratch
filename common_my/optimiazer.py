...
class Nestrov(Optimizer):
    ...
    """
    https://cding.tistory.com/107 참고 
    """
    ...
    def update(self, params, grads):
        if self.v is None:
            ...
            for key in params.keys():
                ...
                params[key] = params[key] + self.momentum*self.v[key] - self.lr*grads[key]
...
class Adam:
    ...
    def update(self, params, grads):
        ...
        self.iter_num += 1
        lr_t = self.lr * np.sqrt(1.0 - self.beta2**self.iter_num) / (1.0 - self.beta1 ** self.iter_num)
        ...
        for key in params.keys():
            ...
            params -= lr_t * self.m / (np.sqrt(self.v[key]) + 1e-7)