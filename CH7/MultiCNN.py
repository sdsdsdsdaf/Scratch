...
class MultiCNN: #CNN으로 시작을 하지 않으면 이미지가 흐름대로 흘러가지 않음
    ...
    """
    def __init__(self, input_dim = (1, 28, 28), layer_list = ['conv', 'relu', 'pooling', 'affine', 'relu', 'affine', 'softmax'],
                 filter_shape_list = [[30, 5, 5, 0, 1]], hidden_layer_list = [[50],[50]], pooling_list = [[2, 2, 2], [2,2,2]], output_size = 10, activation = "ReLu", weight_init_std = 'Reul', weight_decay_lambda=0,
        ...
        try:
            ...
            np.array(filter_shape_list).ndim == 2 and np.array(hidden_layer_list).ndim == 2 and np.array(pooling_list)
        ...
        self.params = {}
        pre_channel_num = input_dim[-3]
        conv_i = 0
        affine_i = 0
        
        for idx, conv_parm in enumerate(filter_shape_list):
            self.params[f'W{idx+1}'] = weight_init_scale[idx] * np.random.randn(filter_shape[idx])
